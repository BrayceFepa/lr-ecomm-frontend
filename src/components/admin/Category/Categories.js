...
const Categories = () => {
  ...
  const submitCategory = async (e) => {
    ...
    try {
      const response = await productsApi.storeCategory(data);
      console.log("store-categ--response", response);
      swal("Success", response.message, "success");
      document.getElementById("CATEGORY_FORM").reset();
    } catch (error) {
      console.log("store-categ-error", error.response.data);
      setCategory({
        ...categoryInput,
        error_list: error.response.data.errors,
      });
      console.log("error_list", categoryInput.error_list);
    }
  };
  ...
  return (
    <div className="container-fluid px-4">
      {display_errors.map((item, idx) => {
        return (
          <p key={idx} className="mb-1 text-danger">
            {item}
          </p>
        );
      })}
      ...
      <div className="card mt-4">
        ...
        <div className="card-body">
          <form
            className="p-3 pb-5 card-body border"
